...
def run():
    ...
    require('PARTICLE_AREA_DIVISOR_DESKTOP = 1750' in JS, "Desktop particle density should be significantly stronger")
    require('PARTICLE_AREA_DIVISOR_MOBILE = 2300' in JS, "Mobile particle density should remain lighter than desktop")
    require('PARTICLE_MIN_DESKTOP = 620' in JS and 'PARTICLE_MAX_DESKTOP = 1000' in JS, "Desktop particle count should be strong but capped")
    require('PARTICLE_MIN_MOBILE = 120' in JS and 'PARTICLE_MAX_MOBILE = 190' in JS, "Mobile particle count should remain performance-aware")
    require('getParticleCount' in JS and 'width * height' in JS, "Particle count should be calculated from viewport area")
    require('alpha: 0.11 + Math.random() * 0.17' in JS, "Particle base opacity should be clearly visible")
    require('highlight: Math.random() < 0.18' in JS, "Particle field should include a stronger brighter depth layer")
    require('radius: 0.5 + Math.random() * 1.15' in JS, "Particles should be slightly larger on average")
