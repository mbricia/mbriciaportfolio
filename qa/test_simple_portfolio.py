...
def run():
    ...
    require('TECH STACK' in TEXT.upper(), "Tech carousel should have a visible TECH STACK label")
    require('class="container tech-carousel-inner"' in HTML, "Tech carousel should be contained to the same page width as other content")
    ...
    require('id="particleBackground"' in HTML, "Portfolio should include a fixed decorative particle canvas")
    require('class="particle-background"' in HTML, "Particle canvas class is missing")
    require("getContext('2d')" in JS, "Particle background should use a lightweight 2D canvas")
    require('PARTICLE_AREA_DIVISOR_DESKTOP' in JS, "Desktop particle density control is missing")
    require('PARTICLE_AREA_DIVISOR_MOBILE' in JS, "Mobile particle density control is missing")
    require("addEventListener('pointermove'" in JS, "Desktop particle background should react subtly to pointer movement")
    require("addEventListener('scroll'" in JS, "Particle background should react subtly to scrolling")
    require('PARTICLE_AREA_DIVISOR_DESKTOP = 3300' in JS, "Desktop particle density should scale with viewport area")
    require('PARTICLE_AREA_DIVISOR_MOBILE = 2700' in JS, "Mobile particle density should scale separately")
    require('PARTICLE_MIN_DESKTOP = 320' in JS and 'PARTICLE_MAX_DESKTOP = 600' in JS, "Desktop particle count should be clamped for performance")
    require('PARTICLE_MIN_MOBILE = 100' in JS and 'PARTICLE_MAX_MOBILE = 160' in JS, "Mobile particle count should be clamped for performance")
    require('getParticleCount' in JS and 'width * height' in JS, "Particle count should be calculated from viewport area")
    require('alpha: 0.08 + Math.random() * 0.14' in JS, "Particle base opacity should be more visible but still subtle")
    require('highlight: Math.random() < 0.14' in JS, "Particle field should include a small brighter depth layer")
    require('opacity:.72' in CSS, "Desktop particle canvas should be clearly visible through the glass UI")
